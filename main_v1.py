...
features = list(pd.read_csv(DATA_FOLDER + '/v1/importances.csv', index_col=0).head(350).index)
...
test = load_test(DATA_FOLDER + '/v1/test.pkl')[features]
...
importance.to_csv(DATA_FOLDER + "/v1/importances_filtered.csv")

prepare_submission(test_target, "v1f_AUC_%.4f_F1_%.4f" % (result['auc'], result['f1']))
